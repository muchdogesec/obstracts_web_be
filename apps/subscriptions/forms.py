...
class UsageRecordForm(forms.Form):
    ...
    subscription_item = forms.ModelChoiceField(
        SubscriptionItem.objects.none(), label=_("Product")
    )
    ...
    def __init__(self, subscription_holder: SubscriptionModelBase, *args, **kwargs):
        ...
        self.metered_plans = (
            subscription_holder.active_stripe_subscription.items.filter(
        ...
        self.fields["subscription_item"].label_from_instance = (
            lambda item: item.price.product.name
        )
