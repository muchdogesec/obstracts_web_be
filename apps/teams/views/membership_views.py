...
def team_membership_details(request, team_slug, membership_id):
    ...
    if not can_edit_team_members and not editing_self:
        messages.error(
            request, _("Sorry, you don't have permission to access that page.")
        )
        return HttpResponseRedirect(
            reverse("single_team:manage_team", args=[request.team.slug])
        )
    ...
    if request.method == "POST":
        ...
        if not can_edit_team_members:
            return HttpResponseForbidden(
                _("You don't have permission to edit team members in that team.")
            )
        ...
        if editing_self:
            ...
            if membership_form.is_valid():
                ...
                messages.success(
                    request,
                    _("Role for {member} updated.").format(
                        member=membership.user.get_display_name()
                    ),
                )
    else:
        membership_form = MembershipForm(instance=membership)
...
def remove_team_membership(request, team_slug, membership_id):
    ...
    if not can_edit_team_members:
        if not removing_self:
            return HttpResponseForbidden(
                _("You don't have permission to remove others from that team.")
            )
    if membership.role == ROLE_ADMIN:
        admin_count = Membership.objects.filter(
            team=request.team, role=ROLE_ADMIN
        ).count()
        if admin_count == 1:
