...
class GetImageUrlFunctionTestCase(SimpleTestCase):
    ...
    def test_absolute_urls_not_touched(self):
        self._run_test(
            "https://example.com/absolute.jpg", "https://example.com/absolute.jpg"
        )

    @override_settings(
        MEDIA_URL="/media/", STATIC_URL="/static/", STORAGES=TEST_STORAGES
    )
    def test_static_resolution(self):
        self._run_test(
            "/static_image.jpg", "https://example.com/static/static_image.jpg"
        )

    @override_settings(MEDIA_URL="/media/")
    def test_with_relative_media_url_prefix(self):
        self._run_test(
            "/media/media_image.jpg", "https://example.com/media/media_image.jpg"
        )
...
"""
        self.assertEqual(
            "The URL is https://example.com/test/1/", self._get_rendered_text(template)
        )
...
"""
        self.assertEqual(
            "The URL is https://example.com/terms/", self._get_rendered_text(template)
        )
